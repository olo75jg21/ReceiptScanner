...
const show = (req, res, next) => {
  ...
  Receipt.paginate({ userId: ObjectId }, options, {
    __v: 0,
    createdAt: 0,
    updatedAt: 0,
  })
...
const update = (req, res, next) => {
  ...
  Receipt.findByIdAndUpdate(receiptId, {
    $set: { data: req.body.data, shop: req.body.shop, price: req.body.price },
  })
    .then(() => {
      res.status(200).json({
...
const updateItem = (req, res, next) => {
  ...
  Receipt.updateOne(
    { _id: receiptId, "receiptItems._id": itemId },
    {
      $set: {
...
const getMonthTotalSum = (req, res, next) => {
  var ObjectId;
  try {
    ObjectId = mongoose.Types.ObjectId(req.params.userId);
  } catch (error) {
  ...
  const endDate = new Date(`${year}-${month}-31T00:00:00.000+00:00`);
  endDate.setMonth(endDate.getMonth() + 1);

  Receipt.find(
    { userId: ObjectId, date: { $gte: startDate, $lt: endDate } },
    (err, receipts) => {
      if (err) {
        res.status(500).json({
          message: "Internal server error",
